	...
	function SlashCommandHandler(command)
		if command then
			...
			elseif command == 'invert' then
				...
				Print('反轉計時條 ' .. (ccc_settings.invert and '開啟。' or '關閉。'))
			...
			elseif args[1] == 'ignore' then
				...
				Print('忽略 "' .. effect_name .. '" ' .. (ccc_settings.ignore[effect_name] and '開啟。 ' or '關閉。'))
			else
				Usage()
			end
		end
...
function Usage()
	Print('指令語法:')
	Print('  鎖定解鎖 : lock | unlock')
	Print('  反向 : invert')
	Print('  延伸方向 : growth (up | down)')
	Print('  放大縮小 : scale [0.5,3]')
	Print('  透明度 : alpha [0,1]')
	Print('  對象 : aoe (target | all)')
	Print('  忽略 : ignore {name}')
	Print('  忽略 : ignore')
end
