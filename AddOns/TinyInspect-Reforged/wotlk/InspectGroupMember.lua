...
LibEvent:attachEvent("CHAT_MSG_ADDON", function(self, prefix, text, channel, sender)
    if (prefix == "TinyInspectReforged") then
...
LibEvent:attachEvent("GROUP_ROSTER_UPDATE", function(self)
    ...
    if (not IsInGroup()) then return TinyInspectReforgedRaidFrame:Hide() end
    ...
    if (numCurrent ~= numMembers) then
        ...
        TinyInspectReforgedRaidFrame:Show()
    end
...
local frame = CreateFrame("Frame", "TinyInspectReforgedRaidFrame", UIParent, "InsetFrameTemplate3")
